...
bool t_syntax::start()
{
  ...
  /*
     if(this->gettree_classes()->getnodes().size() > 1)
     {
     if(this->gettree_classes()->getroot()->getchildren()->back()->getlabel() == L_PROG)
     {
     this->gettree_classes()->getroot()->deletechild();
     }
     }
...
/*
   t_syntaxtree *t_syntax::gettree_functions()
   {
   return syntaxtree_functions;
   }

   t_syntaxtree *t_syntax::gettree_classes()
   {
   return syntaxtree_class;
   }
 */
...
{	
  this->lexical->next_token();
  if(this->lexical->get_current_token() != T_LEFTBRACE)
    return syntaxerror(DEBUG_BLOC, 231, ERROR_BLOC_LEFT_BRACE);
  ...
  if(this->lexical->get_current_token() != T_RIGHTBRACE)
    return syntaxerror(DEBUG_BLOC, 247, ERROR_BLOC_RIGHT_BRACE); 
}
...
void t_syntax::for_instruction(t_syntaxtree *tree, t_node<t_symbol> *syntaxnode)
{
  ...
  { 
    if(this->lexical->next_token() != T_LEFTPARENTHESIS)
      return syntaxerror(DEBUG_FOR, 257, ERROR_FOR_LEFT_PARENTHESIS);

    this->lexical->next_token();
    //this->variable_affectation(tree, syntaxnode, false);
    this->variable(tree, syntaxnode, false, false);

    if(this->lexical->get_current_token() != T_DOTCOMMA)
      return syntaxerror(DEBUG_FOR, 264, ERROR_FOR_FIRST_DOTCOMMA);
    ...
    if(this->lexical->get_current_token() != T_DOTCOMMA)
      return syntaxerror(DEBUG_FOR, 278, ERROR_FOR_SECOND_DOTCOMMA);

    this->lexical->next_token();
    //this->variable_affectation(tree, syntaxnode, true);
    this->variable(tree, syntaxnode, true, false);

    if(this->lexical->get_current_token() != T_RIGHTPARENTHESIS)
      return syntaxerror(DEBUG_FOR, 285, ERROR_FOR_RIGHT_PARENTHESIS);
...
void t_syntax::if_instruction(t_syntaxtree *tree, t_node<t_symbol> *syntaxnode)
{
  ...
  { 
    if(this->lexical->next_token() != T_LEFTPARENTHESIS)
      return syntaxerror(DEBUG_IF, 302, ERROR_IF_LEFT_PARENTHESIS);

    this->lexical->next_token();
    id_var = this->expression(tree, syntaxnode);

    if(this->lexical->get_current_token() != T_RIGHTPARENTHESIS)
      return syntaxerror(DEBUG_IF, 308, ERROR_IF_RIGHT_PARENTHESIS);
    ...
    {
      if(this->lexical->next_token() != T_LEFTPARENTHESIS)
        return syntaxerror(DEBUG_IF, 329, ERROR_IF_LEFT_PARENTHESIS);

      this->lexical->next_token();
      id_var = this->expression(tree, syntaxnode);

      if(this->lexical->get_current_token() != T_RIGHTPARENTHESIS)
        return syntaxerror(DEBUG_IF, 335, ERROR_IF_RIGHT_PARENTHESIS);
...
{	
  if(this->lexical->get_current_token() == T_CLASS)
  {
    ...
    if(this->global_class->find(lexical->get_current_lexeme()) != this->global_class->end())
      return syntaxerror(DEBUG_CLASS, 485, ERROR_CLASS_IDENTIFIER_EXIST);
    ...
    std::map<std::string, t_identifier *> *class_variables = new std::map<std::string, t_identifier *>;
    chained_variables.push_back(class_variables);
    this->global_variables = class_variables;
...
{	
  if(this->lexical->get_current_token() == T_FUNCTION)
  {
    ...
    if(lexical->next_token() != T_IDENTIFIER)
      return syntaxerror(DEBUG_FUNCTION, 521, ERROR_FUNCTION_IDENTIFIER);

    if(global_functions->find(lexical->get_current_lexeme()) != global_functions->end())
      return syntaxerror(DEBUG_FUNCTION, 524, ERROR_FUNCTION_IDENTIFIER_EXIST);
    ...
    std::map<std::string, t_identifier *> *local_variables = new std::map<std::string, t_identifier *>;
    chained_variables.push_back(local_variables);
    this->global_variables = local_variables;

    if(this->lexical->next_token() != T_LEFTPARENTHESIS)
      return syntaxerror(DEBUG_FUNCTION, 553, ERROR_FUNCTION_LEFT_PARENTHESIS);
    ...
    for(it = this->global_variables->begin(); it != this->global_variables->end(); it++)
...
{	  
  if(lexical->get_current_token() == T_RETURN)
  {
    ...
    if(this->lexical->get_current_token() != T_DOTCOMMA) 
      return syntaxerror(DEBUG_RETURN, 600, ERROR_RETURN_DOTCOMMA);
...
void t_syntax::call_instruction(t_syntaxtree *tree, t_node<t_symbol> *syntaxnode, bool isexpression, std::map<std::string, t_function *> *functions_temp)
{
  if(this->lexical->get_current_token() == T_IDENTIFIER)
  {
    if(functions_temp->find(lexical->get_current_lexeme()) == functions_temp->end())
      return syntaxerror(DEBUG_CALL, 615, ERROR_CALL_IDENTIFIER_DOESNT_EXIST);

    t_function *function_found = (*functions_temp)[lexical->get_current_lexeme()];

    if(this->lexical->next_token() != T_LEFTPARENTHESIS)
      return syntaxerror(DEBUG_CALL, 620, ERROR_CALL_LEFT_PARENTHESIS);
    ...
    for(i = 0; i < function_found->getnbargs(); i ++)
    {
      ...
      if(i < (function_found->getnbargs() - 1) && this->lexical->get_current_token() != T_COMMA)
        return syntaxerror(DEBUG_CALL, 641, ERROR_CALL_ARG);
    }
    ...
    if((function_found->getnbargs() && this->lexical->get_current_token() != T_RIGHTPARENTHESIS) || 
      ...
      return syntaxerror(DEBUG_CALL, 646, ERROR_CALL_RIGHT_PARENTHESIS);

    if(isexpression && this->lexical->next_token() != T_DOTCOMMA)
      return syntaxerror(DEBUG_CALL, 649, ERROR_CALL_DOTCOMMA);
...
void t_syntax::variable_declaration(t_syntaxtree *tree, t_node<t_symbol> *syntaxnode, bool paramsfunctions)
{
  ...
  if(lexical->get_current_token() == T_DECLR)
  {
    ...
    if(lexical->next_token() != T_IDENTIFIER)
      return syntaxerror(DEBUG_VARDEC, 675, ERROR_VARDEC_IDENTIFIER);
    ...
    if(this->insideclass && !this->isfunction)
    {
      ...
      if(this->current_classe->getvariables()->find(lexical->get_current_lexeme()) != this->current_classe->getvariables()->end())
        return syntaxerror(DEBUG_FUNCTION, 685, ERROR_FUNCTION_IDENTIFIER_EXIST);
    ...
    else
    {
      ...
      if(this->global_variables->find(lexical->get_current_lexeme()) != this->global_variables->end())
        return syntaxerror(DEBUG_VARDEC, 694, ERROR_VARDEC_IDENTIFIER_EXIST);
    ...
    //t_symbol *symbol1 = new t_symbol(lexical->get_current_line(), lexical->get_current_column(), S_VARIABLE);
    //t_node<t_symbol> *node_var = node->addchild(L_VARIABLE, symbol1);
    //tree->addnode(node_var);
    ...
    if(lexical->next_token() == T_LEFTBRACKET)
    {
      ...
      if(lexical->get_current_token() != T_RIGHTBRACKET)
        return syntaxerror(DEBUG_VARDEC, 719, ERROR_VARDEC_RIGHTBRACKET);
...
void t_syntax::variable_end(t_syntaxtree *tree, t_node<t_symbol> *syntaxnode, bool paramsfunctions)
{
  ...
  if(paramsfunctions)
  {
    if(this->lexical->get_current_token() != T_COMMA && this->lexical->get_current_token() != T_RIGHTPARENTHESIS) 
      return syntaxerror(DEBUG_VAREND, 872, ERROR_VAREND_RIGHT_PARENTHESIS);
  ...
  {
    if(this->lexical->get_current_token() != T_DOTCOMMA) 
      return syntaxerror(DEBUG_VAREND, 881, ERROR_VAREND_DOTCOMMA);
  ...
  if(lexical->get_current_token() == T_VARIABLE)
  {
    ...
    if(lexical->next_token() == T_IDENTIFIER)
    {
      if(this->global_variables->find(lexical->get_current_lexeme()) != this->global_variables->end())
      {
        ...
        {
          isclass = true;
          variables_temp = identifier_class_found->getclass()->getvariables();
    ...
    if(isclass)
    {
      ...
      if(this->lexical->next_token() != T_SUP)
        return syntaxerror(DEBUG_CALL, 770, ERROR_CALL_LEFT_PARENTHESIS);
    ...
    if(variables_temp->find(lexical->get_current_lexeme()) != variables_temp->end())
    {
      ...
      if(lexical->next_token() == T_LEFTBRACKET)
      {
        ...
        id_var = this->expression(tree, syntaxnode);
        if(lexical->get_current_token() != T_RIGHTBRACKET)
          return syntaxerror(DEBUG_VARDEC, 943, ERROR_VARDEC_RIGHTBRACKET);
    ...
    else
    {
      ...
      this->call_instruction(tree, method_node, isexpression, functions_temp);
    }
  }
}
...
void t_syntax::classe_instance(t_syntaxtree *tree, t_node<t_symbol> *syntaxnode, t_identifier *identifier)
{
  ...
  {
    if(this->global_class->find(lexical->get_current_lexeme()) == this->global_class->end())
      return syntaxerror(DEBUG_CLASS_INSTANCE, 850, ERROR_CLASS_IDENTIFIER_DOESNT_EXIST);
...
unsigned int t_syntax::expression(t_syntaxtree *tree, t_node<t_symbol> *syntaxnode)
{
  ...
  t_identifier *identifier = new t_identifier(lexical->get_current_line(), lexical->get_current_column(), S_EXPRESSION, lexical->get_current_lexeme(), this->global_variables->size());
  std::ostringstream out;
  out << "expression" << this->global_variables->size();
  this->global_variables->insert(std::pair<std::string, t_identifier *>(out.str(), identifier));
...
void t_syntax::term(t_syntaxtree *tree, t_node<t_symbol> *syntaxnode)
{
  ...
  std::ostringstream out;
  out << "term" << this->global_variables->size();
...
void t_syntax::factor(t_syntaxtree *tree, t_node<t_symbol> *syntaxnode)
{
  ...
  std::ostringstream out;
  out << "factor" << this->global_variables->size();
  ...
  else if(token == T_VARIABLE)
  {
    ...
    //this->variable(tree, node_factor);
  }
  ...
  else if(token == T_LEFTPARENTHESIS)
  {
    ...
    if(token != T_RIGHTPARENTHESIS)
      return syntaxerror(DEBUG_FACTOR, 1111, ERROR_FACTOR_RIGHT_PARENTHESIS);
